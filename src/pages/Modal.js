...
export default function Modal({ manageUser, users }) {
  const [showModal, setShowModal] = useState(false);
  return (
    <>
      <button
        className=" text-white active:bg-pink-600 rounded hover:shadow-lg outline-none focus:outline-none ease-linear transition-all duration-150"
        // className="bg-pink-500 text-white active:bg-pink-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
        type="button"
        onClick={() => setShowModal(true)}
      >
        <Image src={manageUser} style={{ objectFit: 'contain', maxWidth: '2.5rem', paddingTop: '.25rem', }} alt="add user"></Image>
      </button>
      {showModal ? (
        <>
          <div
